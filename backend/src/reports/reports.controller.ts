...
export class ReportsController {
  ...
  async monthly(@CurrentUser() user: AuthenticatedUser, @Query('month') month?: string) {
    ...
    const totalSaved = goals.reduce((sumGoal, goal) => sumGoal + goal.movements.reduce((s, x) =>
      s + Number(x.amount) * (x.type === 'CONTRIBUTION' ? 1 : -1), 0), 0);
    return {
      month: start, income, expenses, balance: income - expenses - saved,
      overallBalance: overallIncome - overallOut - overallSaving, saved, totalSaved, emergencies,
